...
router.post("/auth/register", async (req, res) => {
  ...
  try {
    ...
    const response = await axios.post(
      `${AUTH_SERVICE_URL}/auth/register`, // Construct the full URL for the Auth Service  
      req.body, // Pass the request 
      {
...
router.post("/auth/login", async (req, res) => {
  try {
    ...
    logger.error({ error: error.message }, "Error during login request"); 
...
// Route to forward /auth/logout requests to auth-service. protect middleware is used to verify the user is logged in before they can log out.
